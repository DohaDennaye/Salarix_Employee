    ...
    public function submitDefineAccess(submitDefineAccessRequest $request, $email)
{
    try {
        ...
        if (!$user) {
            return back()->with('error_msg', 'Paramètre de connexion non reconnu');
        }

        // Vérifier le code envoyé
        $existingCode = ResetCodePassword::where('email', $email)
                                        ->where('code', $request->code)
                                        ->first();

        if (!$existingCode) {
            return back()->with('error_msg', 'Code incorrect');
        }
        ...
        return back()->with('error_msg', 'Une erreur est survenue');
    }
}
}